...
public class DoiPass extends AppCompatActivity {
    ...
    private void DoimatkhauFirebase(final View v, String matkhaucu, final String matkhaumoi) {
        AuthCredential credential=EmailAuthProvider.getCredential(Trangchu.mUser.getEmail(),matkhaucu);
        Trangchu.mUser.reauthenticate(credential).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()){
                    Trangchu.mUser.updatePassword(matkhaumoi).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()){
                                pro_doipass.setVisibility(View.GONE);
                                Toast.makeText(DoiPass.this, "Đổi mật khẩu thành công", Toast.LENGTH_SHORT).show();
                                startActivity(new Intent(DoiPass.this,Trangchu.class));
                            }else{
                                Toast.makeText(DoiPass.this, "Đổi mật khẩu firebase thất bại", Toast.LENGTH_SHORT).show();
                                v.setVisibility(View.VISIBLE);
                                v.setEnabled(true);
                                pro_doipass.setVisibility(View.INVISIBLE);
                            }
